...
Nodes of type `anchor` are currently not used in Ory Kratos, but we have them
around in case they are needed at a later stage!
...
Nodes of type `image` are used as QR codes, for example.
...
Here is an example of implementing an `image` node in handlebars:
...
Here is an example of implementing an `image` node in ReactJS:
